...
This learning lab will explore advanced Docker features for containers, like networking, service stack creation, storage and native orchestration. We would recommend you go through the Docker-101 lab first, so you can build on that foundational knowledge and expand your expertise on Docker.
...
* Microservices: we hear the terms *containers* and *microservices* used a lot together. Microservices benefit from a lightweight packaging, distribution and deployment solution. However, you can  package anything into a container, including a badly written legacy app in some cases, using containers does not magically make bad code better.

* Virtual machines: it sounds obvious, but worth remembering they are different. Containers are purely user-space, if you need kernel extensions/modules or a custom kernel, containers probably are not what you are looking for.

* Magic: they bring their own nuances and require deployment consideration just like any other toolchain.
